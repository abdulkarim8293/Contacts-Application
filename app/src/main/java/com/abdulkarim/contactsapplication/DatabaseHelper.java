...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // get previous monthly expense data and show in the previous monthly expense fragment

    public List<Expanse> getPreviousMonthExpenseDatas(){
        // array of columns to fetch
        String[] columns = {
                COLUMN_EXPANSE_ID,
                COLUMN_EXPANSE_TYPE,
        ...
        // sorting orders
        String sortOrder = COLUMN_EXPANSE_DATE + " ASC";
        String where = " strftime('%Y',expanse_date) = strftime('%Y',date('now')) AND  strftime('%m',expanse_date) = strftime('%m',date('now','-1 month'))";
        //String where = " strftime('%Y',expanse_date) = strftime('%Y',date('now'))";
        List<Expanse> expanseList = new ArrayList<Expanse>();
    ...
    // get yearly expense data and show in the Yearly Expense Fragment

    public List<Expanse> getYearlyExpenseDatas(){
        // array of columns to fetch
        String[] columns = {
                COLUMN_EXPANSE_ID,
                COLUMN_EXPANSE_TYPE,
    ...
    // get current weekly expanse data as total from database;

    public int getCurrentWeekExpanseData(){
        SQLiteDatabase database = this.getReadableDatabase();
        String query ="SELECT SUM(" + COLUMN_EXPANSE_AMOUNT + ") as Total FROM " + TABLE_EXPANSE + " WHERE "+COLUMN_EXPANSE_DATE+" >= DATETIME('now','-6 day')";
    ...
    public int getCurrentWeekIncomeData(){
        SQLiteDatabase database = this.getReadableDatabase();
        String query ="SELECT SUM(" + COLUMN_INCOME_AMOUNT + ") as Total FROM " + TABLE_INCOME + " WHERE "+COLUMN_INCOME_DATE+" >= DATETIME('now','-6 day')";
        Cursor cursor = database.rawQuery(query,null);
        if (cursor.moveToFirst()){
            return cursor.getInt(cursor.getColumnIndex("Total"));
        }
    ...
    // get current days Expanse data and Income data form sqlite

    public int getCurrentDaysExpanseData(){
        SQLiteDatabase database = this.getReadableDatabase();
        String query ="SELECT SUM(" + COLUMN_EXPANSE_AMOUNT + ") as Total FROM " + TABLE_EXPANSE + " WHERE "+COLUMN_EXPANSE_DATE+" >= DATE('now')";
        Cursor cursor = database.rawQuery(query,null);
        if (cursor.moveToFirst()){
            return cursor.getInt(cursor.getColumnIndex("Total"));
        }
    ...
    public int getCurrentDaysIncomeData(){
        SQLiteDatabase database = this.getReadableDatabase();
        String query ="SELECT SUM(" + COLUMN_INCOME_AMOUNT + ") as Total FROM " + TABLE_INCOME + " WHERE "+COLUMN_INCOME_DATE+" >= DATE('now')";
        Cursor cursor = database.rawQuery(query,null);
        if (cursor.moveToFirst()){
            return cursor.getInt(cursor.getColumnIndex("Total"));
        }
    ...
    // get current monthly expense data and income data from sqlite

    public int getCurrentMonthlyExpenseData(){
        SQLiteDatabase database = this.getReadableDatabase();
        String query ="SELECT SUM(" + COLUMN_EXPANSE_AMOUNT + ") as Total FROM " + TABLE_EXPANSE + " WHERE strftime('%Y',expanse_date) = strftime('%Y',date('now')) AND  strftime('%m',expanse_date) = strftime('%m',date('now'))";
        Cursor cursor = database.rawQuery(query,null);
        if (cursor.moveToFirst()){
            return cursor.getInt(cursor.getColumnIndex("Total"));
        }
    ...
    public int getCurrentMonthlyIncomeData(){
        SQLiteDatabase database = this.getReadableDatabase();
        String query ="SELECT SUM(" + COLUMN_INCOME_AMOUNT + ") as Total FROM " + TABLE_INCOME + " WHERE strftime('%Y',income_date) = strftime('%Y',date('now')) AND  strftime('%m',income_date) = strftime('%m',date('now'))";
        Cursor cursor = database.rawQuery(query,null);
        if (cursor.moveToFirst()){
            return cursor.getInt(cursor.getColumnIndex("Total"));
        }
    ...
    // get previous month expense and income data as total from database;
    public int getPreviousMonthlyExpenseData(){
        SQLiteDatabase database = this.getReadableDatabase();
        String query ="SELECT SUM(" + COLUMN_EXPANSE_AMOUNT + ") as Total FROM " + TABLE_EXPANSE + " WHERE strftime('%Y',expanse_date) = strftime('%Y',date('now')) AND  strftime('%m',expanse_date) = strftime('%m',date('now','-1 month'))";
        Cursor cursor = database.rawQuery(query,null);
        if (cursor.moveToFirst()){
            return cursor.getInt(cursor.getColumnIndex("Total"));
        }
    ...
    public int getPreviousMonthlyIncomeData(){
        SQLiteDatabase database = this.getReadableDatabase();
        String query ="SELECT SUM(" + COLUMN_INCOME_AMOUNT + ") as Total FROM " + TABLE_INCOME + " WHERE strftime('%Y',income_date) = strftime('%Y',date('now')) AND  strftime('%m',income_date) = strftime('%m',date('now','-1 month'))";
        Cursor cursor = database.rawQuery(query,null);
        if (cursor.moveToFirst()){
            return cursor.getInt(cursor.getColumnIndex("Total"));
        }
    ...
    // get current yearly expense and income data from database

    public int getCurrentYearlyExpenseData(){
        SQLiteDatabase database = this.getReadableDatabase();
        String query ="SELECT SUM(" + COLUMN_EXPANSE_AMOUNT + ") as Total FROM " + TABLE_EXPANSE + " WHERE strftime('%Y',expanse_date) = strftime('%Y',date('now'))";
        Cursor cursor = database.rawQuery(query,null);
        if (cursor.moveToFirst()){
            return cursor.getInt(cursor.getColumnIndex("Total"));
        }
    ...
    public int getCurrentYearlyIncomeData(){
        SQLiteDatabase database = this.getReadableDatabase();
        String query ="SELECT SUM(" + COLUMN_INCOME_AMOUNT + ") as Total FROM " + TABLE_INCOME + " WHERE strftime('%Y',income_date) = strftime('%Y',date('now'))";
        Cursor cursor = database.rawQuery(query,null);
        if (cursor.moveToFirst()){
            return cursor.getInt(cursor.getColumnIndex("Total"));
        }
    ...
    public void deleteIncome(int id){
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(TABLE_INCOME, COLUMN_INCOME_ID    + "    = ?", new String[] { String.valueOf(id)});
    }
